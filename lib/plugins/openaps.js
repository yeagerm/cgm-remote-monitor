...
function init (ctx) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if ('enacted' === prop.status.code) {
      var basalRate = Number(prop.lastEnacted.rate);
      var hasBasalRate = Number.isFinite(basalRate);
      var hasDuration = prop.lastEnacted.duration !== undefined && prop.lastEnacted.duration !== null;
      var hasTempBasalDetails = hasBasalRate && hasDuration;
      var canceled = hasTempBasalDetails && basalRate === 0 && prop.lastEnacted.duration === 0;
      ...
      if (canceled || hasTempBasalDetails) {
        ...
        if (!canceled) {
          valueParts.push(' ' + basalRate.toFixed(2) + ' for ' + prop.lastEnacted.duration + 'm');
        }
      }
      valueParts.push(
